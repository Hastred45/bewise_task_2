...
def get_user_by_id_and_UUID(id: int,
                            UUID: str,
...
async def wav_to_mp3(audio_file: UploadFile) -> bytes:
...
async def _upload_media(id: int, UUID: str,
    ...
    user = get_user_by_id_and_UUID(id, UUID, db)
    if user is None:
        raise HTTPException(
            status_code=404,
    ...
    try:
        mp3_file = await wav_to_mp3(audio_file)
